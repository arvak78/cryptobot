...
public class ProjectReader {
    ...
    private  void readRecursive(Map<ModuleName, File> modules, File current) throws IOException {
        ...
        // El padre no lo añadimos
        if (!rootPom.getAbsolutePath().equals(current.getAbsolutePath())) {
        ...
        for (String child : children) {
            ...
            if(current.getParent() == null) {
                ...
                childPom = new File(filePath, child);
            } else {
                String absolutePath = current.getParentFile().getAbsolutePath();
                childPom = new File(absolutePath, child);
            }
